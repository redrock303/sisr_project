...
def main():
    ...
    train_loader = dataloader.train_loader(train_dataset, config, is_dist=distributed)
    if rank <= 0:
    ...
    for lr_img, hr_img in train_loader:
        ...
        if rank <= 0:
            ...
            if iteration % config.VAL.PERIOD == 0 or iteration == config.SOLVER.MAX_ITER:
                ...
                model.eval()
                psnr, ssim = validate(model, val_loader, device, iteration, down=config.MODEL.DOWN,
                                      to_y=config.VAL.TO_Y, save_path=save_dir, save_img=config.VAL.SAVE_IMG,
                                      max_num=config.VAL.MAX_NUM)
